...
path1 = "/workspace/host/Experiments/Synthetic/Synthetic_other/ROSENBROCK/ALEBO/"
            ...
            for D in DD:
                for rando in randoo:
                    ...
                    for no_rand in rand_flag:
                        if(no_rand==0):
                            no_f = [5, 15, 30]
                            mu_rng = np.vstack((np.arange(0,no_f[-1]),np.ones(f_rng))).T
                            
                        else:
                            no_f = [5, 15, 30]
                            mu_rng = np.vstack((np.array(random.sample(range(0, D), f_rng)),np.ones(f_rng))).T
