...
def ei_or_nei(
    ...
    if (
        ...
        return qExpectedImprovement(model=model, best_f=best_f, maximize=maximize)
    else:
...
#Batch Acquisition Function in OracleBO
...
def alebo_acqf_optimizer(
    ...
    for i in range(n):
        ...
        Yrnd1 = torch.matmul(Xrnd, B.t())  # Project down to the embedding
        Yrnd = torch.reshape(Yrnd1,(int(Yrnd1.shape[0]/q),q,B.shape[0]))
        ...
        with gpytorch.settings.max_cholesky_size(2000):
            ...
            if(q != 1):
                cand, acq_value = fsamp(candidate= (torch.pinverse(B) @ candidate.T).T,acq_value=acq_value,q=q,mu=mu,sig=sig)
                candidate = (B @ cand).reshape(1,-1)
                #print(candidate.shape)
...
#Dimension Matched Sampler in OracleBO
def fsamp(
    candidate: Tensor,
    mu: Dict[int, float],
    acq_value: Tensor,
    ...
    cand_pdf = (acq_value.numpy())*np.ones(q)
    for i in mu:
        cand_pdf = cand_pdf * scipy.stats.norm(mu[i],sig).pdf(candidate[:,i])
        #print(cand_pdf)
    cc = np.argmax(cand_pdf)
    return (candidate[cc],acq_value)
    
    """
    cand_list = []
    #print(candidate.shape, mu, acq_value, q, sig)
    for j in range(q):
        cand_pdf = acq_value.numpy()
        for i in mu:
            #print(i)
            cand_pdf *= scipy.stats.norm(mu[i], sig).pdf(candidate[j][i])
        print(cand_pdf)
    cand_list.append(cand_pdf)
    cc = cand_list.index(max(cand_list))
    """
...
class ALEBO(BotorchModel):
    ...
    def __init__(
        self, B: Tensor, q: int, mu: Dict[int, float], sig: float, laplace_nsamp: int = 25, fit_restarts: int = 10
    ) -> None:
        self.B = B
        # pyre-fixme[4]: Attribute must be annotated.
